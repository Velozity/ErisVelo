...
class Message extends Base {
  constructor(data, client) {
    ...
    switch (this.type) {
      ...
      default: {
        this._client.emit(
          "warn",
          `Unhandled MESSAGE_CREATE type: ${JSON.stringify(data, null, 2)}`
        );
        break;
      }
    }
  ...
  get cleanContent() {
    let cleanContent =
      (this.content && this.content.replace(/<a?(:\w+:)[0-9]+>/g, "$1")) || "";
  ...
  get jumpLink() {
    return `${Endpoints.CLIENT_URL}${Endpoints.MESSAGE_LINK(
      ...
      this.channel.id,
      this.id
    )}`; // Messages outside of guilds (DMs) will never have a guildID property assigned
  }
