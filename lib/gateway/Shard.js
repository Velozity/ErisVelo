      ...
      case "VOICE_STATE_UPDATE": {
          ...
          if (packet.d.channel_id === null) {
            ...
            if (!flag) {
              for (const privateChannel of this.client.privateChannels) {
                const call =
                  privateChannel[1].call || privateChannel[1].lastCall;
                if (call && call.voiceStates.remove(packet.d)) {
                  flag = true;
                  break;
                }
            ...
            if (!channel.call && !channel.lastCall) {
              this.emit(
                "debug",
                new Error("VOICE_STATE_UPDATE for untracked call")
              );
              break;
            }
        ...
        if (!member) {
          ...
          if (channel) {
            ...
            this.emit(
              "debug",
              "VOICE_STATE_UPDATE member null but in channel: " + packet.d.id,
              this.id
            );
          }
        }
        ...
        if (oldChannelID != packet.d.channel_id) {
          ...
          if (oldChannelID) {
            ...
            if (
              ...
              this.emit(
                "warn",
                "Old channel not a recognized voice channel: " + oldChannelID,
                this.id
              );
              oldChannel = null;
            }
        ...
        if (!guild) {
          this.emit(
            ...
            `Received GUILD_MEMBERS_CHUNK, but guild ${packet.d.guild_id} is ` +
              (this.client.unavailableGuilds.has(packet.d.guild_id)
                ? "unavailable"
                : "missing"),
            this.id
          );
        ...
        if (this.client.user.bot) {
          ...
          if (packet.d.user_guild_settings) {
            packet.d.user_guild_settings.forEach((guildSettings) => {
              this.client.userGuildSettings[guildSettings.guild_id] =
                guildSettings;
            });
          }
      ...
      case "PRESENCES_REPLACE": {
        for (const presence of packet.d) {
          ...
          if (!guild) {
            this.emit(
              "debug",
              "Rogue presences replace: " + JSON.stringify(presence),
              this.id
            );
            continue;
          }
